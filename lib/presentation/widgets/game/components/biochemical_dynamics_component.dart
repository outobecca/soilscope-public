...
class BiochemicalDynamicsComponent extends Component
    with HasGameReference<SoilScopeGame>, RiverpodLifecycleMixin {
        ...
        if (_random.nextDouble() < 0.01 * activityFactor * dt) {
          ...
          if (consumed) {
            ...
            game.moleculePool?.spawn(
              type: isCarbon
                  ? MoleculeType.labileCarbon
                  : MoleculeType.organicNitrogen,
              position: pos,
              velocity: vel,
  ...
  void _spawnNitrogenCycleFlow(double dt) {
    ...
    for (final plant in state.plants) {
      ...
      final activityFactor = layer != null
          ? BiophysicsUtils.q10Factor(layer.temperature).clamp(0.1, 4.0)
          : 1.0;
  ...
  void _spawnGasExchangeFlows(double dt) {
    ...
    if (_random.nextDouble() < 0.025 * photosynthesisRate * dt) {
      ...
      final foliageTarget = Vector2(
        plantWorldPos.x + game.soilLeftX,
        surfaceY - visualHeight * 0.6,
      );
    ...
    if (_random.nextDouble() < 0.01 * photosynthesisRate * dt) {
      ...
      final startPos = Vector2(
        plantWorldPos.x + game.soilLeftX + (_random.nextDouble() - 0.5) * 40,
        surfaceY - visualHeight * 0.7,
      );
  ...
  void _spawnWaterCycleFlows(double dt) {
    ...
    final surfaceY = game.soilSurfaceY;
    final plant = state.plants.first;
    final transpirationActivity = (plant.actualTranspiration * 200000.0).clamp(
      0.1,
      1.5,
    );
    ...
    if (_random.nextDouble() < 0.025 * transpirationActivity * dt) {
      ...
      if (tips.isNotEmpty) {
        ...
        final startPos = targetTip.clone()
          ..add(Vector2((_random.nextDouble() - 0.5) * 60, 100));
  ...
  void _spawnBubbles(double dt) {
    ...
    for (final layer in state.profile.layers.take(3)) {
      ...
      final gasConcentration =
          layer.co2Content + layer.methaneContent + layer.nitrousOxideContent;
      final saturation = layer.waterContent / layer.porosity;

      // Bubbles spawn easier in wet soil (diffusion blocked)
      final spawnChance = (gasConcentration * 0.02 * saturation).clamp(
        0.002,
        0.1,
      ); // REDUCED: from 0.05 / 0.005 / 0.2
      ...
      if (_random.nextDouble() < spawnChance * dt * 0.75) {
        ...
        final success = game.ref
            .read(simulationProvider.notifier)
            .consumeGas(layer.id, symbol, amount);
  ...
  void _syncMolecularParticles() {
    ...
    for (int l = 0; l < topLayers.length; l++) {
      ...
      final counts = {
        ParticleType.nitrate: _toVisualCount(
          layer.nitrateContent,
          factor: 0.04 * depthFactor,
          max: 4,
        ),
        ParticleType.ammonium: _toVisualCount(
          layer.ammoniumContent,
          factor: 0.04 * depthFactor,
          max: 4,
        ),
        ParticleType.organicNitrogen: _toVisualCount(
          layer.organicNitrogen,
          factor: 0.04 * depthFactor,
          max: 3,
        ),
        ParticleType.labileCarbon: _toVisualCount(
          layer.labileCarbon,
          factor: 0.03 * depthFactor,
          max: 3,
        ),
        ParticleType.water: _toVisualCount(
          layer.waterContent * 20.0,
          factor: 0.01 * depthFactor,
          max: 3,
        ),
      };

      final isolate = game.ref
          .read(simulationProvider.notifier)
          .particleIsolate;
      counts.forEach((type, count) {
        ...
        if (count > current / topLayers.length) {
          ...
          if (toAdd > 0) {
            ...
            for (int i = 0; i < toAdd; i++) {
              ...
              double py =
                  surfaceY +
                  (layer.depth / 1.0) * soilHeight +
                  _random.nextDouble() * ((layer.thickness / 1.0) * soilHeight);
          ...
          isolate.removeParticles(
            (current / topLayers.length - count).round(),
            type.index,
          );
        }
      });
