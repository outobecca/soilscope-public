...
class MolecularParticleFieldComponent extends Component
  ...
  void onMount() {
    super.onMount();
    final isolateManager = game.ref.read(simulationProvider.notifier).particleIsolate;
  ...
  void render(Canvas canvas) {
    ...
    try {
      for (int i = 0; i < _particleData.length; i += 10) {
        ...
        if (px < visibleRect.left || px > visibleRect.right || py < visibleRect.top || py > visibleRect.bottom) {
        ...
        double baseOpacity = (isImmobilized ? 0.35 : 0.55) * (life > 0 ? life : 1.0).clamp(0.0, 1.0);
        ...
        bool isPartOfActiveCycle = false;
        switch (activeCycle) {
          case ObservationCycle.nitrogen:
            isPartOfActiveCycle = type == ParticleType.nitrate || type == ParticleType.ammonium || type == ParticleType.organicNitrogen;
            break;
          case ObservationCycle.carbon:
            isPartOfActiveCycle = type == ParticleType.carbon || type == ParticleType.labileCarbon || type == ParticleType.stableCarbon;
            break;
          case ObservationCycle.water:
            isPartOfActiveCycle = type == ParticleType.water;
            break;
          case ObservationCycle.phosphorus:
            isPartOfActiveCycle = type == ParticleType.phosphorus || type == ParticleType.organicPhosphorus;
            break;
          case ObservationCycle.none:
            isPartOfActiveCycle = true;
            break;
        }
        ...
        if (activeCycle != ObservationCycle.none && isPartOfActiveCycle) {
          ...
          if (s == 'N' || s == 'NITROGEN') {
            ...
            isElementHighlighted = type == ParticleType.phosphorus ||
                type == ParticleType.organicPhosphorus;
          }
        }
        ...
        if (isPinned || isHovered || isElementHighlighted) {
          ...
          final double scaledRadius = (baseHighlightRadius / zoom).clamp(baseHighlightRadius, 25.0);
          ...
          canvas.drawCircle(
            ...
            Paint()
              ...
              ..style = PaintingStyle.stroke
              ..strokeWidth = (isElementHighlighted ? 2.0 : 1.2) / zoom,
          );
        ...
        if (zoom < 0.6 && !isPaused && !isPinned && !isHovered) {
          final Paint basePaint = _getSimplifiedPaint(pTypeIdx, isImmobilized, baseOpacity);
          ...
          if (isDark) {
             canvas.drawCircle(pos, (isImmobilized ? 4.5 : 4.0), Paint()..color = Colors.white.withValues(alpha: 0.15));
          }
          canvas.drawCircle(
            pos,
        ...
        } else if (pTypeIdx == ParticleType.labileCarbon.index || pTypeIdx == ParticleType.carbon.index) {
          _drawLabileCarbon(canvas, pos, px + py, opacity: baseOpacity, zoom: zoom);
        } else if (pTypeIdx == ParticleType.stableCarbon.index) {
          canvas.drawCircle(
            pos,
            3.2,
            _stableCarbonPaint..color = _stableCarbonPaint.color.withValues(alpha: baseOpacity * 0.7),
          );
        } else if (pTypeIdx == ParticleType.water.index) {
          _drawWater(canvas, pos, px - py, opacity: baseOpacity, zoom: zoom);
        } else if (pTypeIdx == ParticleType.organicNitrogen.index) {
          _drawOrganicNitrogen(canvas, pos, px + py, opacity: baseOpacity, zoom: zoom);
        ...
        if (!isImmobilized && !isPaused && zoom > 1.2) { 
          _drawParticleTrail(canvas, px, py, pvx, pvy, pTypeIdx, zoom);
        }
      }
    } catch (e) {
      // Robustness
  ...
  void _showParticleInfoFromIndex(int index, {bool pinned = false}) {
    ...
    switch (type) {
      ...
      case ParticleType.phosphorus:
        title = "PHOSPHATE";
        desc = "Essential nutrient for plant growth, transferred by mycorrhizae.";
        break;
      case ParticleType.organicPhosphorus:
        title = "ORGANIC PHOSPHORUS";
        desc = "Phosphorus bound in organic matter. Requires phosphatase enzymes to unlock.";
        break;
    }
  ...
  Paint _getSimplifiedPaint(int typeIndex, bool isImmobilized, double alpha) {
    if (isImmobilized) {
       return _immobilizedPaint..color = _immobilizedPaint.color.withValues(alpha: alpha);
    }
    final type = ParticleType.values[typeIndex];
    switch (type) {
      case ParticleType.ammonium:
        return _ammoniumPaint..color = _ammoniumPaint.color.withValues(alpha: alpha);
      case ParticleType.nitrate:
        return _nitratePaint..color = _nitratePaint.color.withValues(alpha: alpha);
      case ParticleType.labileCarbon:
      case ParticleType.carbon:
        return _labileCarbonPaint..color = _labileCarbonPaint.color.withValues(alpha: alpha);
      case ParticleType.stableCarbon:
        return _stableCarbonPaint..color = _stableCarbonPaint.color.withValues(alpha: alpha);
      case ParticleType.water:
  ...
  void _drawAmmonium(
    ...
    // 1. Shadow for depth
    canvas.drawCircle(center + const Offset(0.5, 0.5), 4.5, Paint()..color = Colors.black.withValues(alpha: 0.2 * opacity));

    // 2. Main Nitrogen Core
    canvas.drawCircle(center, 4.5, Paint()..color = currentColor.withValues(alpha: opacity)); 
    
    // 3. Highlight for 3D look
    canvas.drawCircle(center - const Offset(1.2, 1.2), 1.8, Paint()..color = Colors.white.withValues(alpha: opacity * 0.4));
    ...
    for (int i = 0; i < 4; i++) {
      ...
      canvas.drawCircle(p, 1.3 + morphProgress * 0.25, Paint()..color = hColor.withValues(alpha: opacity * 0.9));
      canvas.drawCircle(p - const Offset(0.3, 0.3), 0.4, Paint()..color = Colors.white.withValues(alpha: opacity * 0.6));
    }
    ...
    if (isPaused && zoom > 2.0) { 
      ...
      final cPaint = _chargePaint..color = Colors.white.withValues(alpha: 0.95 * opacity);
      canvas.drawLine(Offset(center.dx - 1.5, center.dy), Offset(center.dx + 1.5, center.dy), cPaint);
      canvas.drawLine(Offset(center.dx, center.dy - 1.5), Offset(center.dx, center.dy + 1.5), cPaint);
    }
  }
  ...
  void _drawNitrate(
    ...
    // 2. Central Nitrogen
    canvas.drawCircle(center, 4.5, Paint()..color = colorN.withValues(alpha: opacity));
    canvas.drawCircle(center - const Offset(1.2, 1.2), 1.8, Paint()..color = Colors.white.withValues(alpha: opacity * 0.4));
    ...
    for (int i = 0; i < 3; i++) {
      ...
      canvas.drawCircle(p, 1.5, Paint()..color = colorO.withValues(alpha: opacity * 0.9));
      canvas.drawCircle(p - const Offset(0.4, 0.4), 0.5, Paint()..color = Colors.white.withValues(alpha: opacity * 0.6));
    }
  ...
  void _drawChargeLabel(Canvas canvas, Offset center, String symbol, Color color, double zoom) {
    canvas.drawCircle(center, 6.0, Paint()..color = const Color(0xFF0F172A).withValues(alpha: 0.8));
    canvas.drawCircle(center, 6.0, Paint()..color = color.withValues(alpha: 0.5)..style = PaintingStyle.stroke..strokeWidth = 1.0);

    final stroke = 1.5 / zoom; // Thicker stroke
    final paint = Paint()..color = color.withValues(alpha: 1.0)..style = PaintingStyle.stroke..strokeWidth = stroke;

    const s = 5.0;
    canvas.drawLine(Offset(center.dx - s / 2, center.dy), Offset(center.dx + s / 2, center.dy), paint);
    if (symbol == '+') {
      canvas.drawLine(Offset(center.dx, center.dy - s / 2), Offset(center.dx, center.dy + s / 2), paint);
    }
  }
  ...
  void _drawPhosphorus(
    ...
    canvas.drawCircle(center, 2.8, Paint()..color = colorP.withValues(alpha: opacity));
    ...
    for (int i = 0; i < 4; i++) {
      ...
      final p = Offset(center.dx + dist * math.cos(angle), center.dy + dist * math.sin(angle));
      canvas.drawCircle(p, 1.2, Paint()..color = colorO.withValues(alpha: opacity * 0.7));
    }
  ...
  void _drawLabileCarbon(Canvas canvas, Offset center, double phase, {double opacity = 1.0, required double zoom}) {
    final colorC = CPKStandards.colorC;
    final colorO = CPKStandards.colorO;
    
    _drawGlow(canvas, center, 10.0, colorC.withValues(alpha: 0.15 * opacity));
    canvas.drawCircle(center, 3.5, Paint()..color = colorC.withValues(alpha: opacity));
    ...
    if (zoom > 1.3) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        canvas.drawCircle(p, 1.8, Paint()..color = colorO.withValues(alpha: opacity * 0.8));
      }
    } else {
  ...
  void _drawWater(Canvas canvas, Offset center, double phase, {double opacity = 1.0, required double zoom}) {
    final colorO = CPKStandards.colorO;
    final colorH = CPKStandards.colorH;
    
    if (zoom > 1.5) {
      // High-fidelity CPK Water (Bent molecule)
      canvas.drawCircle(center, 3.2, Paint()..color = colorO.withValues(alpha: opacity));
      final dist = 3.8;
      for (int i = 0; i < 2; i++) {
        final angle = (i == 0 ? 0.6 : 2.5) + phase * 0.005;
        final p = center + Offset(math.cos(angle) * dist, math.sin(angle) * dist);
        canvas.drawCircle(p, 1.4, Paint()..color = colorH.withValues(alpha: opacity * 0.9));
      }
    } else {
      ...
      canvas.drawCircle(center, 3.2 / zoom, Paint()..color = colorO.withValues(alpha: opacity));
    }
  }

  void _drawOrganicNitrogen(Canvas canvas, Offset center, double phase, {double opacity = 1.0, required double zoom}) {
    final colorN = CPKStandards.colorN;
    final colorC = CPKStandards.colorC;
    
    // Removed glow for clear CPK rendering
    canvas.drawCircle(center, 3.8, Paint()..color = colorN.withValues(alpha: opacity));
    ...
    if (zoom > 1.4) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        canvas.drawCircle(p, 2.2, Paint()..color = colorC.withValues(alpha: opacity * 0.8));
      }
    }
  }

  void _drawParticleTrail(Canvas canvas, double px, double py, double vx, double vy, int typeIndex, double zoom) {
